...
object KafkaManagedOffsetCache {
  val supportedVersions: Set[KafkaVersion] = Set(Kafka_0_8_2_0, Kafka_0_8_2_1, Kafka_0_8_2_2)
        ...
        for {
          ...
          while (!shutdown) {
            try {
              ...
              val GroupTopicPartition(group, topicAndPartition) = OffsetManager.readMessageKey(ByteBuffer.wrap(messageAndMetadata.key()))

              val value = OffsetManager.readMessageValue(ByteBuffer.wrap(messageAndMetadata.message()))
              val OffsetAndMetadata(offset, metadata, timestamp) = value

              val topic = topicAndPartition.topic

              groupTopicPartitionOffsetMap += (group, topicAndPartition.topic, topicAndPartition.partition) -> value
              val consumerSet = {
